...
interface Order {
  _id: string;
  restaurantId: string;
  tableId: { displayName: string; tableNumber: string } | any; // resolved table or plain ID
  orderNumber: number;
  items: OrderItem[];
  subtotal: number;
  tax: number;
...
export const ManagerOrders: React.FC = () => {
  ...
  const { data: activeData, isLoading: isLoadingActive } = useQuery({
    ...
    refetchInterval: 10000, // Poll active orders every 10 seconds (before we add sockets in Phase 6!)
  });
  ...
  const orders: Order[] = activeTab === 'active' ? activeData?.data || [] : historyData?.data?.orders || [];
    ...
    <div className="max-w-6xl mx-auto px-4 py-8 font-sans">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
        ...
        <div className="flex border border-slate-200 rounded-xl bg-slate-50 p-1 shrink-0">
          ...
          >
            Live Active Queue ({activeTab === 'active' ? orders.length : '...'})
          </button>
          <button
            ...
            return (
              ...
              >
                ...
                <div className="p-5 bg-slate-50/50 border-t border-slate-50 flex gap-3">
                  {/* Cancel button: blocked for STAFF */}
                  {!isStaff && (order.status === 'PENDING' || order.status === 'ACCEPTED') && (
                    <button
                      ...
                      className="flex-1 py-2.5 border border-red-200 text-red-600 hover:bg-red-50 text-xs font-bold rounded-xl transition"
                    >
                      Cancel Order
                    </button>
                  )}
                  ...
                  {nextStatus ? (
                    <button
                      ...
                      className="flex-1 py-2.5 bg-slate-900 hover:bg-slate-800 text-white text-xs font-bold rounded-xl transition flex items-center justify-center gap-1 shadow-sm"
                    >
