...
export const PublicTable: React.FC = () => {
  ...
  return (
    <div style={cssVariables} className="min-h-screen bg-slate-50 font-sans antialiased pb-28 relative">
      ...
      {activeTab === 'landing' && (
        ...
        >
          ...
          <div className="max-w-md mx-auto px-4 space-y-6">
            ...
            {/* Description */}
            <div className="space-y-2">
              <h4 className="text-xs font-extrabold text-slate-400 uppercase tracking-widest block font-mono">
                About Restaurant
              </h4>
            ...
            <button
              ...
              className="w-full py-4 bg-slate-950 hover:bg-slate-800 text-white font-extrabold text-sm rounded-2xl transition flex items-center justify-center gap-2 shadow-md"
            >
          ...
          <div className="max-w-md mx-auto px-4 space-y-8">
            {isMenuLoading ? (
              ...
              <div className="text-center py-12 bg-white rounded-2xl border border-slate-100 p-8 space-y-2">
                <HelpCircle className="w-10 h-10 text-slate-400 mx-auto" strokeWidth={1.75} />
                <h3 className="text-sm font-bold text-slate-800">No items available</h3>
                <p className="text-xs text-slate-400">All categories are currently sold out.</p>
              </div>
            ) : (
      ...
      {/* -------------------- 3. SEARCH & FILTERS TAB -------------------- */}
      {activeTab === 'search' && (
        <motion.div
          key="search"
          initial={{ opacity: 0, y: 15 }}
          animate={{ opacity: 1, y: 0 }}
          className="max-w-md mx-auto px-4 py-4 space-y-6"
        >
          {/* Main search panel */}
          <div className="relative">
            <SearchIcon className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-slate-400" strokeWidth={1.75} />
            <input
              type="text"
          ...
          <div className="bg-white p-4 rounded-2xl border border-slate-150 shadow-sm space-y-3.5">
            ...
            <div className="flex justify-between items-center text-xs">
              <span className="font-bold text-slate-500">Price Sort</span>
              <select
                value={priceSort}
                onChange={(e) => setPriceSort(e.target.value as any)}
                className="border border-slate-200 rounded-xl px-2.5 py-1.5 bg-slate-50 focus:outline-none font-semibold text-slate-700"
              >
            ...
            <div className="flex justify-between items-center text-xs pt-1">
              <span className="font-bold text-slate-500">In Stock Only</span>
              <button
                type="button"
                onClick={() => setShowAvailableOnly(!showAvailableOnly)}
                className={`w-10 h-6 rounded-full transition-colors relative flex items-center px-1 shrink-0 ${
                  showAvailableOnly ? 'bg-amber-500' : 'bg-slate-200'
                }`}
              >
                <div
          ...
          <div className="space-y-4">
            {isMenuLoading ? (
              ...
              <div className="text-center py-12 bg-white rounded-2xl border border-slate-150 p-8 space-y-2">
                <HelpCircle className="w-10 h-10 text-slate-300 mx-auto animate-bounce" strokeWidth={1.75} />
                <h3 className="text-sm font-bold text-slate-800">No matched menu items</h3>
                <p className="text-xs text-slate-400">Try adjusting your query or filters.</p>
              </div>
            ) : (
              <div className="grid grid-cols-1 gap-3">
                {filteredCategories.flatMap((c) => c.menuItems).map((item) => (
                  <div
                    key={item._id}
                    onClick={() => handleItemCardClick(item)}
                    className={`flex gap-4 p-4 bg-white rounded-3xl border transition-all ${
                      item.isAvailable
                        ? 'border-slate-150 hover:border-slate-300 shadow-sm cursor-pointer'
                        : 'border-slate-100 bg-slate-50 opacity-60 cursor-not-allowed'
                    }`}
                  >
                    ...
                    <div className="flex-1 flex flex-col justify-between py-0.5">
                      <div>
                        <div className="flex justify-between items-start gap-1">
                          ...
                          <span className="shrink-0">{item.isVegetarian ? <MenuBadge variant="veg" /> : ''}</span>
                        </div>
                        {item.description && <p className="text-[10px] text-slate-400 line-clamp-1 mt-0.5">{item.description}</p>}
                      </div>
                      <div className="flex justify-between items-center mt-1">
                        <span className="text-xs font-bold font-mono text-slate-800">{formatPrice(item.price, currency)}</span>
                        {item.isAvailable && <span className="text-[9px] font-bold text-amber-600 bg-amber-50 rounded-lg px-2 py-0.5 uppercase">Add</span>}
                      </div>
                    </div>
                  </div>
                ))}
      ...
      {activeTab === 'reviews' && (
        ...
        >
          ...
          <a
            href={restaurant.googleReviewUrl || `https://www.google.com/search?q=${encodeURIComponent(restaurant.name + ' Review')}`}
            target="_blank"
            rel="noreferrer"
      ...
      <nav className="fixed bottom-0 left-0 right-0 h-16 bg-white border-t border-slate-150 flex items-center justify-around px-4 pb-safe z-40 shadow-lg select-none">
        ...
        <button
          ...
          className={`flex flex-col items-center justify-center gap-1 flex-1 h-full transition-all ${
            activeTab === 'search' ? 'text-slate-950 font-black' : 'text-slate-400 font-semibold'
          }`}
        >
      ...
      <AnimatePresence>
        {cartTotalItems >= 1 && (
          <motion.div
            ...
            className="fixed bottom-20 left-1/2 -translate-x-1/2 z-40 w-full max-w-md px-4 pointer-events-auto"
          >
            <div
              onClick={() => setIsCartSheetOpen(true)}
              className="bg-slate-900 text-white p-4 rounded-2xl shadow-xl flex items-center justify-between cursor-pointer active:scale-[0.98] transition-all border border-white/10"
            >
              <div className="flex items-center gap-3">
                <div className="relative bg-white/10 p-2.5 rounded-xl">
                  ...
                  <motion.span
                    ...
                    className="absolute -top-1.5 -right-1.5 bg-amber-500 text-slate-950 font-mono font-black text-[10px] w-5 h-5 rounded-full flex items-center justify-center shadow"
                  >
                    {cartTotalItems}
                  </motion.span>
                </div>
              ...
              <div className="flex items-center gap-1 text-xs font-bold tracking-wider text-amber-500 bg-white/5 py-1.5 px-3 rounded-lg border border-white/5">
                View Cart
